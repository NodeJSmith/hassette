...
`Api` methods (REST calls to HA) and `StateProxy` access raise `ResourceNotReadyError` while the WebSocket is down. Code that calls these during a disconnect must handle that exception or wait for reconnection.
