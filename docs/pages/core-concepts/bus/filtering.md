...
`A.get_service_data_key` extracts a specific key from service data. `A.get_path` follows a dot-separated path through the full event, so it needs `where=` rather than `changed_to`/`changed_from` — those two narrow the event down to a single extracted state value before the condition ever runs, and a path accessor has nothing to walk once that narrowing happens. Most filtering needs are met by `changed_to`, `changed_from`, and the typed predicates. `A` handles the cases they do not reach. The full accessor guide is at [Custom Extractors](custom-extractors.md).
