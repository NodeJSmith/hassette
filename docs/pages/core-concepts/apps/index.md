...
[`AppSync`][hassette.app.app.AppSync] runs automations written without `async`/`await`. Hassette executes the app's lifecycle hooks in a thread pool so blocking code does not stall the event loop. The bus, scheduler, and API expose synchronous facades via `.sync` (`self.bus.sync`, `self.scheduler.sync`, `self.api.sync`), so registrations and calls work without `await`. Domain entity objects expose the same pattern: `entity.sync` returns a typed `{Domain}EntitySyncFacade` that makes every domain-specific action callable without `await` — `cover.sync.set_cover_position(position=60)`, `climate.sync.set_temperature(temperature=21.0)`, and so on.
