...
If you have blocking code that you cannot convert, use `AppSync` (described below).
...
**Entity states** are typed objects. `self.states.get("light.kitchen")` returns a `LightState` with typed fields. Your IDE knows the shape; Pyright catches typos at development time, not at 2am.
