...
[`D.StateNew[states.BinarySensorState]`](../core-concepts/bus/dependency-injection.md) is a [dependency injection](../core-concepts/bus/dependency-injection.md) annotation — Hassette inspects the handler's parameter types at registration and passes the extracted value in automatically. `D.StateNew` delivers the new state, already converted to a [`BinarySensorState`](../core-concepts/states/index.md) object. `BinarySensorState.value` is a `bool`: `True` when the sensor is on (motion detected), `False` when off (motion cleared) — not the raw HA strings `"on"` and `"off"`. The handler covers both transitions in one place rather than two separate subscriptions.
