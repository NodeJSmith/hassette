...
describe("ExecutionTable", () => {
  ...
  it("renders table with provided testid", () => {
    const { getByTestId } = render(
      <ExecutionTable records={[createExecution()]} kind="job" tableId="execution-table-99" />,
    );
  ...
  it("shows error type inline with status for error rows", () => {
    const { container } = render(
      <ExecutionTable
        records={[createExecution({ status: "error", error_type: "ValueError", error_message: "Task failed" })]}
        kind="job"
        tableId="t"
      />,
    );
  ...
  it("shows context section for handler invocations with trigger_context_id", () => {
    const { container } = render(
      <ExecutionTable
        records={[createInvocation({ trigger_context_id: "ctx-abc-123" })]}
        kind="handler"
