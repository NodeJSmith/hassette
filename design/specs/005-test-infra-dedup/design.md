...
The test infrastructure has accumulated duplication debt: ~130 local `make_*/build_*` factory functions scattered across 400 test files, while `test_utils/factories.py` (built to absorb them) holds only 3 factories. The same conceptual object (`ScheduledJob`, `Event`, `CommandExecutor` mock) is built 4-11 different ways across sibling files. Factory names are overloaded — `make_job` and `make_manifest` each mean 2-3 different return types depending on which file you're in.
...
Additionally, 13 of 19 test app files in `tests/data/apps/` are dead code (verified by full test suite run — 8662 passed after deletion), 4 test_utils exports have zero callers, and 4 fixtures/conftest items sit in the wrong directory level.
...
- Replacing `hassette_instance` in `tests/integration/conftest.py` with `HassetteHarness` — the highest-severity audit finding, but it touches ~32 test functions across 4 files and warrants its own spec
...
- **FR#2** `test_utils/factories.py` exports `make_mock_executor()` — returns a `MagicMock` with `execute = AsyncMock()`
...
- **FR#9** Dead exports are removed: `emit_service_event()` from `helpers.py`, `make_listener_metric()` and `setup_registry()` from `web_helpers.py`, `hassette_with_nothing` fixture from `fixtures.py`, `mock_transport_builder` fixture from `cli/conftest.py`. The dangling `"hassette_with_nothing"` string in `_HARNESS_FIXTURES` (`tests/integration/conftest.py`) and its reference in `TESTING.md:119-123` are also removed
- **FR#10** Dead `@pytest.mark.asyncio` markers are removed from 6 files where `asyncio_mode = "auto"` makes them no-ops
...
- **FR#12** `noop()` moves from `tests/unit/scheduler/conftest.py` to `test_utils/helpers.py` (imported by 7 files, and 5 additional files reimplement a byte-identical local `async def noop() -> None: pass` — `test_scheduler_mode.py`, `test_database_service.py`, `test_scheduler_resource.py`, `test_scheduled_job.py`, `test_task_bucket.py` — which should also be migrated to the shared import)
- **FR#13** `tests/unit/conftest.py::make_test_config` is renamed to `make_sync_executor_config` (collides with the public `test_utils.config.make_test_config`)
...
- **Local factory with same name but different return type**: `make_job` in `test_web_utils.py` returns a `MagicMock` with only a `trigger` attribute — this is legitimately different from the shared `make_scheduled_job()` which returns a real `ScheduledJob`. The linter must support an exemption mechanism for these cases.
...
- **`make_mock_parent()` has 4 variants with slightly different field sets**: `tests/unit/bus/conftest.py` sets `app_key`, `index`, `unique_name`, `source_tier`, `class_name` (omits `app_config`); `tests/unit/bus/test_bus_timeout_threading.py` and `tests/unit/scheduler/conftest.py` both set only `app_key`, `index`, `source_tier`, `class_name` (omit both `unique_name` and `app_config`). The shared factory includes all fields with defaults — callers that didn't set those fields get harmless extra attributes.
...
- **AC#1** `grep -rn "def make_job\b" tests/` returns exactly 3 results: the 2 legitimately distinct MagicMock-based local variants (`test_web_utils.py` and `test_scheduler_service_error_handler.py`) plus the unrelated nested `make_job(label, signal)` in `tests/integration/test_scheduler.py:177` (returns a callable, not a `ScheduledJob` — not in consolidation scope). The 9 real-`ScheduledJob` variants that existed before are gone. (FR#1, FR#7)
...
- **AC#3** `grep -rn "def make_executor\b" tests/` returns only the 2 legitimately-different real-`CommandExecutor` builders (`tests/unit/core/conftest.py` and `tests/unit/core/test_command_executor_pipeline.py`), not the 4 identical mock versions (FR#2, FR#7)
...
- **AC#5** `grep -rn "emit_service_event\|make_listener_metric\|setup_registry\|hassette_with_nothing\|mock_transport_builder" src/hassette/test_utils/ tests/unit/cli/conftest.py` returns zero results (FR#9)
- **AC#6** `grep -rn "pytest.mark.asyncio" tests/system/ tests/unit/core/` returns zero results (FR#10)
...
| `make_mock_executor()` | `MagicMock` | 4 byte-identical `make_executor()` | 4 files |
| `make_mock_event()` | `MagicMock(spec=Event)` | 4 byte-identical `make_event()` | 4 files |
...
| `make_mock_parent(**kw)` | `MagicMock` | 5 `def` variants (conftest, bus/conftest, scheduler/conftest, bus/test_bus_timeout_threading, test_scheduler_resource) + 3 inline constructions | 8 sites across 7 files |
...
No specific implementation preferences — follow codebase conventions. The linter follows `tools/check_internal_patches.py`'s pattern. Factories follow `factories.py`'s existing style.
...
| 4 local `make_executor()` | `make_mock_executor()` in `factories.py` | Delete local definitions, add imports |
...
| 4 `def make_mock_parent()` copies + 3 inline `mock_parent = Mock()` constructions | `make_mock_parent()` in `factories.py` | Delete local defs, replace inline constructions with factory call |
...
- **modify** `src/hassette/test_utils/helpers.py` — delete `emit_service_event`, add `noop`
...
- **modify** `tests/unit/cli/conftest.py` — delete `mock_transport_builder`
- **modify** `.pre-commit-config.yaml` — add `check_test_factories.py` hook
- **modify** ~24 unique test files — replace local factory definitions with imports from shared factories (deduplicated: some files need multiple factories replaced; includes `tests/unit/bus/test_bus_timeout_threading.py` for `make_mock_parent` and 5 files with local `noop()` reimplementations)
...
Factory consolidation touches ~24 unique test files but each change is mechanical (delete local `def`, add `from hassette.test_utils.factories import ...`). No production code is modified. The linter and rule file are additive. Risk is low.
