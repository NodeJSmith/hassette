...
- **FR#11** The WebSocket handler rejects an unauthenticated/untrusted connection with close code `1008` before calling `accept()`.
- **FR#12** The frontend WebSocket reconnect logic stops its backoff loop on close code `1008` and redirects to the login view, instead of retrying indefinitely.
...
Whether `websocket.cookies` is reliably populated at this pre-accept point under this project's specific `ws="websockets-sansio"` uvicorn backend (`core/web_api_service.py:71`, a deliberate non-default choice) has not been empirically verified — see Open Questions.
...
`frontend/src/hooks/use-websocket.ts` currently retries on every close with exponential backoff and no branch on `event.code` (`onclose` handler at `use-websocket.ts:148-158`, calling `scheduleReconnect()` at `use-websocket.ts:165-169`). This changes to check for `event.code === 1008` and, on that condition, stop the backoff loop and redirect to the new login view — rather than retrying indefinitely against a connection that a fresh cookie won't fix.
