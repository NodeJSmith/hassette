...
`ApiDep`): a plain accessor function (e.g. `get_auth`) plus `AuthDep = Annotated[X,
Depends(get_auth)]`, giving route handlers access to whatever T02/T04 resolution state they need
(the resolved token, cookie mint function, etc.) — follow the existing accessors' exact shape (they
return an attribute off `request.app.state.hassette`).
...
cookie-set case and the incorrect-token → 401 case.
