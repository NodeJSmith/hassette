...
**Element-level entries (8):** See design doc inventory table for exact selectors and JS. Key details:
- Sidebar: `[data-testid='sidebar']` on `/apps`
- Status bar: `[data-testid='status-bar']` on `/apps`
- Command palette: `[data-testid='cmd-palette']` on `/apps` — JS dispatches Ctrl+K, `wait_for: "[data-testid='cmd-palette']"`
- Column picker popover: `[data-testid='column-picker-popover']` on `/logs` — JS clicks `[data-testid='column-picker']` first
- Error spotlight: `[data-testid='overview-error-spotlight']` on `/apps/demo_stimulator/overview`
- Handler error row: `[data-testid^='unified-row-']` on `/apps/demo_stimulator/handlers` — JS finds first row with `[data-testid='handler-failed-count']`
- Instance switcher: `[data-testid='instance-switcher']` on `/apps/motion_lights/overview`
- Log drawer: `[data-testid='log-detail-drawer']` on `/logs` — JS clicks first `tbody tr`
...
7. Poll `GET {DEMO_HASSETTE_URL}/api/apps` until the response contains at least one app with error data (check for non-zero `failed` or error count in the response). Use a **90-second timeout** with 2-second poll interval. `urllib.request` is fine (script is stdlib-only except for yaml).
