...
- **AC#22** An integration test proves a config that passes model validation but leaves the app not running returns a success response reporting `failed` status — not a 422 — with the override persisted. A second proves the partial case: a multi-instance app where the *untargeted* instance fails reports the targeted instance as running with `manifest_status` degraded, so the edit is not reported as having failed. (FR#15)
...
- **AC#30** An integration test proves a config write against a running app completes rather than deadlocking on the app-key lock, asserted under `asyncio.wait_for` so a regression that reintroduces a reload-under-lock surfaces as a timeout instead of a hung suite. (FR#15)
- **AC#31** A CLI test proves `set` renders a `failed` targeted-instance result with the status and the revert command, and that a `degraded` `manifest_status` with a healthy targeted instance renders as secondary context rather than as a failed edit. (FR#15)
...
- **A config write contends for the same per-app-key lock the file watcher already uses.** `reload_app` holds that lock for a full restart, and `initialize_instances` / `shutdown_instances` (`core/app_lifecycle_service.py:163-225`, `:327-344`) process instances sequentially under `anyio.fail_after`, so an N-instance app can hold it for up to `N × (app_shutdown_timeout_seconds + app_startup_timeout_seconds)` — minutes with the defaults if instances time out. That hold is pre-existing; what this design adds is a new, user-triggered caller entering that contention. Because `apply_changes` also iterates apps sequentially under a global `_change_event_lock` (`:698-782`), a slow restart from either side can delay other apps queued in the same file-watcher changeset. The write request has no HTTP-level timeout bounding the client's wait. Accepted rather than bounded: adding a reload timeout would create a new failure mode whose recovery story — override persisted, reload outcome unknown — is worse than a slow response for a single-operator tool. The broader serialization gap is #1227.
...
**Locking.** The write handler holds the per-app-key lock (`_get_app_key_lock`) across **version check, merge, and persist only**, then releases it and reloads through the ordinary public `reload_app()` like any other caller.

That hold is what the correctness argument actually needs. It makes check-then-persist atomic per `(app_key, instance_name)`, so a second writer carrying a stale write-generation token is rejected at its check and never reaches a reload. Two writers cannot both pass their version check for the same instance.

**Do not widen the hold to span the reload.** An earlier draft did, reasoning that writer A could otherwise observe writer B's freshly-`RUNNING` instances and report success for a config that is no longer the one running. That race is not reachable: the token already excludes the same-instance case, and the status report reads `AppRegistry.get_instances()` — a synchronous dict copy (`core/app_registry.py:179-181`) with no `await` between the handler's own `reload_app()` returning and the read, so under cooperative scheduling nothing can interleave there regardless of lock scope. The widened hold bought no guarantee the narrow one lacks, while extending a lock the file watcher's reconciliation also contends for across a full multi-instance restart.

**Do not call `reload_app()` while holding the lock.** Since **#1605** `reload_app` acquires `_get_app_key_lock` itself (`core/app_lifecycle_service.py:629`), as do `start_app` (`:480`) and `stop_app` (`:572`). It is a plain non-reentrant `asyncio.Lock` with no timeout, so calling `reload_app` from inside the hold deadlocks permanently — a hung request, not a slow one. Release first, then reload. This is why the sequence below is ordered as it is, and it is the reason **no `_reload_app_unlocked` extraction is needed**: nothing here has to run the restart from inside the lock, so the shared lifecycle path stays untouched.

The config-write path lives in its own module, `core/config_write_coordinator.py`, alongside the store and the provenance predicate — not as another method on `AppLifecycleService`, which is already 1058 lines against the project's 800-line ceiling. The narrowed lock is what makes that placement possible: the coordinator needs only the app-key lock and the public `reload_app()`, both reachable from an injected `AppLifecycleService`, with no reach into lifecycle internals. It takes the lifecycle service as a constructor dependency and runs this sequence:

1. Admission check — the same `REJECT_IF_UNRELEASED` semantics `reload_app` applies, raising `AppBootstrapNotReleasedError` rather than waiting, which is the behavior the `409` needs. `WAIT_FOR_RELEASE` would block a user's request indefinitely and is never used here. Because `_admit_start` is private to `AppLifecycleService`, the lifecycle service exposes a narrow public accessor for this rather than the coordinator reaching into it.
2. Acquire the app-key lock for the version check, the merge, and the atomic persist that bumps the write-generation token. Release it.
3. `await lifecycle.reload_app(app_key)` — the ordinary public entry point, which takes the lock itself and keeps its existing swallow-and-log behavior, so a reload that throws cannot escape as an unhandled `500` after the override is already on disk.
4. Read `registry.get_instances(app_key)` for the status report.

The route reaches it through `AppHandler`, not directly. `AppHandler` is documented as a thin facade that "delegates all lifecycle operations" (`core/app_handler.py:1-4`), and every existing mutation route calls `hassette.app_handler.<method>` — a repo-wide grep finds zero `app_handler.lifecycle` call sites in `web/` or `cli/`. The config write gets a one-line passthrough alongside `start_app` / `stop_app` / `reload_app` rather than being the first caller to reach past the facade.
...
**The targeted instance's own status is the answer to "did my save work."** The response carries it as a `ResourceStatus`, read straight off that instance's `InstanceEntry` from `get_instances()`. This is the field a client renders as the outcome of the edit.

That has to be its own field rather than being inferred from an app-key-level status, because a write restarts every instance of the app key (see Edge Cases) while targeting exactly one. `ManifestStatus.DEGRADED` is computed as `has_running and has_failed` across *all* tracked instances (`core/app_registry.py:270-278`), with no notion of which instance was just written or when any given failure began — so a write to instance 2 that comes up cleanly still yields `DEGRADED` when instance 5 was already broken for an unrelated reason. Keying the client's "did this work" treatment off that would attribute pre-existing failures to the edit.

The response therefore carries three things:

- `instance` — the targeted instance's index, name, and resulting `ResourceStatus`. The edit's verdict.
- `manifest_status` — the app key's post-reload `ManifestStatus`, deliberately named so it cannot be mistaken for the edit's result. Ambient context, letting the UI reuse the status treatment already wired into the apps view. Note `DEGRADED` is structurally impossible for a single-instance app, since it requires at least two entries.
- `instances` — per-instance detail for every slot, so a user editing one instance of a multi-instance app can see which others failed and that those failures are not theirs.

Reusing `ManifestStatus` as a *value* keeps the dashboard and this response speaking one vocabulary. Reusing it as the headline field would not: every other use of `ManifestStatus` in the codebase is a steady-state read (`web/models.py:146,399`), and the existing mutation-response convention (`ActionResponse`, `web/models.py:454-459`) deliberately reports only `accepted` rather than claiming post-action state.

A client renders a `422` as field-level errors, a targeted-instance `failed` as a "saved, but the app didn't start" notice with a revert affordance, and a `degraded` `manifest_status` alongside a healthy targeted instance as context — other instances need attention — never as a failure of the edit.
...
**The CLI reports the post-write status too.** The write route returns the targeted instance's status plus `manifest_status` and per-instance detail regardless of which surface called it, so `set`/`unset` must render that rather than printing a bare success. The targeted instance drives the headline: `running` is an ordinary success line; `failed` prints the status and the revert command for the fields just written, so the recovery action is on screen at the moment it is needed rather than requiring a separate `app health` call; `stopped` states that the override was saved and applies on next start, so a user who deliberately stopped the app is not told it failed. A `degraded` `manifest_status` alongside a healthy targeted instance prints a secondary line naming which *other* instances are down, explicitly not attributed to this edit.
...
- **Holding the app-key lock across the reload and the status report.** This was the design until adversarial review. It aimed to stop two writers interleaving, but the write-generation token already excludes the same-instance case at the version check, and the status report reads a synchronous dict copy with no `await` after the handler's own reload, so nothing can interleave there regardless of lock scope. It bought no guarantee the narrow hold lacks, forced a `_reload_app_unlocked` extraction onto the shared reload path to avoid deadlocking on the non-reentrant lock, and extended contention with the file watcher across a full multi-instance restart. The cost of narrowing is that two writers targeting *different* instances of one app key can both pass their checks, so the second reload picks up both overrides and a bad value from one writer can surface as a failed report to the other. That needs two clients editing one app inside a single restart window, and the response's per-instance detail shows which slot actually failed.
- **Orchestrating the write inside `AppLifecycleService`.** The lock lives there, so it was the path of least resistance. Rejected: that file is already 1058 lines against the project's 800-line ceiling, and this feature already puts its storage and provenance concerns in their own modules. Narrowing the lock removed the only reason the orchestration needed lifecycle internals, leaving a coordinator module that depends on two public accessors.
...
- `src/hassette/core/config_write_coordinator.py` — **create**: the validate → persist → reload → report orchestration, holding the app-key lock through persist only. Keeps this out of `app_lifecycle_service.py`, already 1058 lines against the 800-line ceiling.
- `src/hassette/core/app_lifecycle_service.py` — **modify**: construct the override store and the write coordinator; expose narrow public accessors for the app-key lock and the admission check. No change to `reload_app` itself.
- `src/hassette/core/app_handler.py` — **modify**: one-line passthrough for the config write, matching the existing `start_app` / `stop_app` / `reload_app` delegations. Routes call the facade, never `app_handler.lifecycle` directly.
...
- `reload_app` is not modified at all — the write path calls the existing public method. The file watcher and the existing start/stop/reload routes are unaffected.
...
The config-write orchestration lives in its own module rather than in `AppLifecycleService`, so the only change to the shared lifecycle path is two narrow public accessors — nothing on the reload path itself. `AppFactory.create_instances` is on every app-creation path, so a defect there affects all app startup, not just apps with overrides — the highest-risk change in the feature. The shared reload path is untouched — the write path calls the existing public `reload_app()` rather than extracting a new entry point into it. `web/config_view.py` is shared by the global config page and the per-app tab, so descriptor changes touch both. The shared atomic-write extraction touches auth token writing. Everything else is additive.
