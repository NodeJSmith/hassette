...
- modify: `src/hassette/web/routes/bus.py` — `get_listener_metrics` still has an explicit
  `except DB_ERRORS` (its CM migration is deferred to T05). Swap **only its catch type** here to
  `TelemetryUnavailableError` so deleting `DB_ERRORS` doesn't leave a dangling import; the CM
  migration stays in T05.
...
3. **Read-path audit + cover bypasses.** Grep the whole `TelemetryQueryService` — the three mixins
   (`registration_queries.py`, `summary_queries.py`, `execution_queries.py`) **and** `query_service.py`
   itself — for `self._db.execute` used **outside** the `execute()` helper. The known bypass is
   `get_all_app_summaries` (`summary_queries.py:215-228`), which runs a manual `BEGIN
   DEFERRED`/`ROLLBACK` transaction directly on `self._db`. Wrap that transaction so the same storage
   tuple is translated to `TelemetryUnavailableError` (preserve the existing `ROLLBACK` on failure,
   then re-raise as the domain error). **Scope rule:** translate only *read paths that serve HTTP
   handlers*. Do **not** translate startup/lifecycle code — e.g. the `PRAGMA journal_mode` call in
   `on_initialize` (`query_service.py:~49`) intentionally raises `RuntimeError` on a bad DB at
   startup; wrapping it would change startup-failure semantics. If the audit surfaces it, leave it
   as-is. Confirm no HTTP-serving read path is left untranslated.

4. **Narrow the HTTP catch.** Change `db_degrades_to` (in `dependencies.py`) to
   `except TelemetryUnavailableError`. Change every remaining explicit `except DB_ERRORS` in `web/`
   to `except TelemetryUnavailableError`: the category-C/D inline catches (`apps.py:get_app_manifests`,
   the three `dashboard_app_grid` sub-queries, and the two `executions.py` sites including the
   `check_retention_expired_uuid4` helper) **and** `bus.py:get_listener_metrics` (still explicit;
   its CM migration is deferred to T05 — only swap the catch type here). Then **delete the
   `DB_ERRORS` tuple** from `dependencies.py` and remove its imports across `web/` (including
   `bus.py:9`) — the storage tuple is now named only inside `execute()` (and the
   `get_all_app_summaries` wrapper).
