...
2. For `scheduled_jobs` (FK parent, requires rebuild):
   ...
   - Rebuild indexes and views (`active_scheduled_jobs`, `active_framework_scheduled_jobs`, `active_all_scheduled_jobs`) — update their `WHERE` clauses from `cancelled_at IS NULL` to `removed_at IS NULL`.
3. Update `active_listeners`, `active_framework_listeners`, `active_all_listeners` views from `cancelled_at` to `removed_at`.

**Update persistence code:**

- `repository.py`: rename `mark_job_cancelled()` → `mark_job_removed()`, update all `cancelled_at` references to `removed_at`. The `register_job()` upsert's `ON CONFLICT DO UPDATE SET` must explicitly include `schedule_status = excluded.schedule_status, schedule_status_reason = excluded.schedule_status_reason` alongside the existing `cancelled_at = NULL` → `removed_at = NULL` clearing.
...
- `registration.py`: update `ScheduledJobRegistration` and `ListenerRegistration` field names.
- `database_service.py`: update retention queries (`WHERE retired_at IS NOT NULL AND retired_at < ?` references to `cancelled_at` become `removed_at`).
...
**Add `TriggerDbType` value:**
Add `"manual"` to the `TriggerDbType` enum/literal in the registration typing so manual jobs can be persisted with `trigger_type="manual"`.
...
- `database_service.py:622-639` has retention queries using `retired_at` that also reference `cancelled_at` in variable names — update those.
