...
3. Convert all methods from `async def` to `def`: `add_route`, `remove_route`, `remove_listener_by_id`, `clear_owner`, `get_topic_listeners`, `get_listeners_by_owner`.
...
## Focus
- `src/hassette/bus/router.py` is 192 lines. All 6 public methods are `async def` with `async with self.lock:`. The lock protects operations that are already atomic in asyncio's single-threaded model.
...
## Verify
- [ ] FR#8: All Router mutation and query methods (`add_route`, `remove_route`, `remove_listener_by_id`, `clear_owner`, `get_topic_listeners`, `get_listeners_by_owner`) are plain `def` with no `await` in their bodies
