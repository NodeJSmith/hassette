...
## Prompt
**1. Update TabId** in `frontend/src/pages/app-detail.tsx`: Find `export type TabId = "handlers" | "code" | "logs" | "config"` and add `"overview"` to the union.

**2. Change default tab** in `frontend/src/pages/app-detail.tsx`: Find `params.tab ?? "handlers"` and change to `params.tab ?? "overview"`.

**3. Add route** in `frontend/src/app.tsx`: Add `<Route path="/apps/:key/overview">` following the existing tab route pattern (the `/apps/:key/code`, `/apps/:key/logs` routes nearby). Place it before the catch-all `/apps/:key` route.

**4. Update tab bar** in `frontend/src/pages/app-detail.tsx`: Find the `<div class="ht-tab-strip" role="tablist">` block containing the `<Tab>` components. Add `<Tab id="overview" label="overview" {...tabProps} />` as the FIRST tab, before the existing handlers tab.

**5. Add render branch** in `frontend/src/pages/app-detail.tsx`: Find the conditional block `{activeTab === "handlers" && (` and add a similar block above it for `{activeTab === "overview" && (` that renders the new `OverviewTab` component.
...
- Find the test "handlers tab is selected by default when no params.tab provided" and change it to assert the overview tab is selected by default
