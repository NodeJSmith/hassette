...
1. **Write `tools/check_internal_patches.py`** mirroring the structure of the existing `tools/frontend/check_*.py` guards (module docstring describing prohibited symbols + the exemption annotation; `main()` returning a nonzero exit on violations; clear `file:line — symbol` output). It must:
   ...
   - Treat a flagged line as exempt when that **same line** carries a trailing `# boundary-exempt:` comment — this matches the convention examples (all annotations are same-line trailing comments) and design.md FR#7 ("the line carries"). Do not accept preceding-line-only annotations; the annotators in T02–T04 write same-line comments. `task_bucket.spawn` is NOT in the prohibited set (different object).
...
- [ ] AC#4: The PR's `system` and `e2e` workflow runs show green (check the run URLs on the PR) — the hard merge gate, confirmed after all conversions land, not assumed from general CI policy. These exercise the real aiohttp/HA boundaries the converted unit tests mock.
- [ ] AC#6: The guard exists and is wired into `.github/workflows/lint.yml`; running it locally passes.
- [ ] AC#7: A GitHub tracking issue exists (has a number) for the scheduler `__new__` doubles in `test_scheduler_service_reschedule.py`; AND the deferred Cluster D and E/F items are recorded in a durable artifact (a GitHub issue or a committed appendix in `design.md`) — verifiable as "the record exists and names each deferred item."
