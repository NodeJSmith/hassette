...
- The `hassette.testing.wait_for` naming collision is accepted — kept `Bus.wait_for` over a `wait_for_event` rename. Not just a "different namespace" argument (module-path proximity in a docs search is a real but mild collision) but a different *access pattern*: `self.bus.wait_for(...)` is a bound method reached through `self.bus.`, while `hassette.testing.wait_for` is a free function reached through a top-level import — the two never compete for the same IDE-autocomplete slot, only for a docs-search query string. `bus.wait_for` also reads better than `bus.wait_for_event` — "event" is redundant once the call is already namespaced under an event bus. If either name should move, `hassette.testing.wait_for` is the better target later — it's the newer, lower-stakes name with far fewer callers to migrate — but no rename is planned now.
- **`TaskBucket.run_sync`'s `timeout_seconds` default was changed from `None` (meaning "use the config value") to the `NOT_PROVIDED` sentinel** (`hassette.const.misc.NOT_PROVIDED`, the same sentinel `Bus.on_attribute_change` already uses for "not specified vs. legitimate value"). `None` now means `run_sync`'s own genuine "block forever" — the same spelling `concurrent.futures.Future.result(timeout=None)` uses internally. This deleted the `float("inf")` translation layer the sync-facade `wait_for` forwarding originally needed, collapsing the codegen special case (`codegen/src/hassette_codegen/sync_facade/generic.py`) to a plain `timeout_seconds=timeout` forward. **Breaking change:** `run_sync` is public, documented API (`hassette.task_bucket` is in the docs' `PUBLIC_MODULES` allowlist, and an advanced task-bucket doc snippet calls it directly). Grepping every internal call site (~65, across the Bus/Api/Scheduler/HelperClient sync facades) confirms only `wait_for`'s own forwarding ever passed `timeout_seconds` explicitly, so this is behavior-preserving for all internal callers — but an external app author who explicitly passed `timeout_seconds=None` to get the config default (rather than omitting the argument, the documented pattern) will now get "block forever" instead. No internal code or doc snippet demonstrates that usage, so the risk is judged low; ships with a `BREAKING CHANGE:` changelog footer regardless, per `changelog-quality.md`.
...
2. **`wait_for` method** — follows the `send_and_await_response` pattern:
   ...
   - Log at DEBUG when a `wait_for` future resolves (matched event) and when it times out. The `_on_listener_removed` cleanup path logs at WARNING when cancelling a pending `wait_for` future outside of shutdown (e.g. an explicit `Subscription.cancel()`), including the count of futures still pending. During `Bus.on_shutdown()`, individual cancellations are counted instead of logged; one summary WARNING ("Cancelled N pending wait_for future(s) during shutdown") is emitted after `remove_all_listeners()` completes, if N > 0 — this keeps routine teardown (an app cleanly stopping with several `wait_for` calls in flight) from flooding the log with one WARNING per future and burying the FR#11 leak-threshold WARNING under noise.
