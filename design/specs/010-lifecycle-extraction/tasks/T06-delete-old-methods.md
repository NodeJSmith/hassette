...
- **Circular import in base.py:** T03 could not migrate `self._run_hooks(...)` and `self._ordered_children_for_shutdown()` in `base.py` because `operations.py` imports `Resource` from `base.py`, creating a circular import. When deleting `_run_hooks` and `_ordered_children_for_shutdown` from `base.py` in this task, you must resolve this — e.g., by using a lazy import inside the function body, or by restructuring the import chain. The 2 call sites are in `Resource.initialize()`, `Resource.shutdown()`, and `Resource.cleanup()`.
- **Two unmigrated call sites found by T04:** (1) `src/hassette/resources/operations.py`'s `start_children_and_wait` still calls `child.start()` (bound method copied verbatim by T01) instead of module-level `start(child)`. (2) `src/hassette/server.py`'s SIGTERM handler registers `core.request_shutdown` by bound-method reference. Both will break when old methods are deleted — fix in this task.
- **Patch target correction:** The design doc's instruction to patch at the defining module (`hassette.resources.lifecycle.func`) is wrong for calls that use `from hassette.resources.lifecycle import func` — those must patch at the calling module (e.g., `hassette.core.websocket_service.mark_ready`). T04 already applied this correction. Keep this in mind when reviewing test changes.
