...
- **Hint arrives before DB write completes:** `LogCaptureHandler.emit()` fires synchronously, before the async DB persistence queue commits the record. The hint carries no cursor — it's just a "go fetch" signal. The frontend's fetch uses its own `lastSeenId` from prior REST responses. If the fetch fires before the new record is committed, it returns the already-committed records; the record appears on the next fetch — possible one-debounce-window delay on a single record.
...
- **AC#9** If a catch-up fetch returns a max `id` below the current `lastSeenId`, the cursor resets and a one-time "log stream reset" notice is surfaced. (FR#8, edge case: DB id regression)
