...
6. **Run the doc reviews** — per `.claude/rules/doc-rules.md` (Verify with Persona and Accuracy Reviews), run the `/doc-persona-review` and `/doc-accuracy-review` skills scoped to the new `core-concepts/blocking-io-detection` page (pass the page slug as the argument). A `lost`/`stuck-at-step-N` persona verdict or a confirmed `WRONG`/`OUTDATED_API` accuracy finding on the new content is a blocker — fix before done.
