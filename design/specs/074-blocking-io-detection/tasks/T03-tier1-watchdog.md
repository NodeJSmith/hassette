...
status: "planned"
depends_on: ["T01", "T02"]
implements: ["FR#1", "FR#2", "FR#3", "FR#9", "FR#12", "AC#1", "AC#3", "AC#9"]
---
...
4. **Install/teardown** — install the watchdog in `src/hassette/core/core.py` `run_forever` after `self._loop_thread_id` is captured (line 449), gated on `blocking_io.watchdog_enabled`. Tear it down on shutdown so no task/thread remains (FR#12, AC#9 for the watchdog tier). Install must be idempotent (double-install is a no-op). If the `HassetteHarness` (`src/hassette/test_utils/harness.py`, task factory at line 534) needs a parallel install hook so harness-based integration tests bring the watchdog up through the normal startup path, add it here — do not leave harness wiring implicit.
...
- [ ] FR#12: Watchdog install is idempotent and teardown leaves no task/thread running — a test asserts a double-install is a no-op and that nothing remains after shutdown (watchdog tier; Tier 2 patches are T04).
- [ ] AC#9: After `Hassette` shuts down, no watchdog task/thread remains, and a second start re-installs the watchdog cleanly (watchdog tier).
