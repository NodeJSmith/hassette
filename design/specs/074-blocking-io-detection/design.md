...
The make-or-break detail (flagged in the research) is attribution. Two candidate mechanisms exist, and **the first work package is a throwaway spike to choose between them** before the design commits:

- **Candidate A — in-loop `call_later` heartbeat.** Schedule a recurring heartbeat (e.g. 250ms); lag beyond threshold means the loop was blocked. Simple and loop-agnostic, but the heartbeat only runs *after* the block clears, by which point the execution context is unbound — so it needs a record of "the handle that just ran and how long it held the loop."
- **Candidate B — dedicated daemon watchdog thread.** A thread observes a thread-visible "last loop tick" timestamp and a thread-visible "currently executing" marker. Because it runs off-loop, it can read the marker *during* the block (the handler's `finally` hasn't run yet) and snapshot `sys._current_frames()[loop_thread_id]` live. Correct attribution by construction; cost is one daemon thread.

Both depend on a new **thread-visible execution marker** (a plain attribute, not a ContextVar — a separate OS thread cannot read another thread's ContextVar, which is why the marker cannot be `CURRENT_EXECUTION_ID` itself). It is set in `command_executor.bind_execution_context` (line 422) on entry and cleared in the paired `unbind_execution_context` static method (line 439, called from the `finally` of both `execute_handler` line 483 and `execute_job` line 520), carrying `(app_key, instance_name, execution_id, started_at)`. The spike validates that the chosen candidate attributes a `time.sleep` stall to the correct app under the "next execution scheduled immediately after" condition (AC#2). The spike's outcome is recorded back into this design before implementation proceeds.
...
- Wired into `HassetteConfig` as `blocking_io: BlockingIODetectionConfig = Field(default_factory=...)`, next to `asyncio_debug_mode` (`config.py:167`).
...
- [ ] **Watchdog mechanism (Candidate A vs B)** — resolved by the WP1 spike before implementation proceeds. The design commits to whichever attributes correctly under AC#2; this section is updated with the outcome.
...
- [ ] **Tier 1 default threshold values** — `lag_threshold_seconds` and `watchdog_interval_seconds` defaults need a sane starting point (research suggests ~250ms interval, ~100ms lag threshold); confirm during the spike against real timing.
