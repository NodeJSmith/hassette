...
## Summary
Create the four new sub-struct dataclasses in `src/hassette/bus/listeners.py` and refactor the existing `Listener` class to compose them. Move `dispatch()`, `invoke()`, `mark_fired()`, and `set_app_error_handler_resolver()` methods from `Listener` to `HandlerInvoker`. Update `Listener.create()` to construct sub-structs internally while preserving backward compatibility for existing kwargs callers. Add backward-compat property accessors on Listener so that T02/T03 pass tests before consumers are migrated in T04/T05. Add `create_cancel_listener()` dedicated factory. Update `Listener.cancel()` to also call `self.invoker.mark_fired()`. Write unit tests for each sub-struct independently.
...
- `HandlerInvoker` — `@dataclass(slots=True)` with 9 fields: `orig_handler`, `_async_handler`, `_injector`, `kwargs`, `error_handler`, `_app_error_handler_resolver`, `_rate_limiter`, `once` (default False), `_fired` (default False, init=False). The `once` field is copied from `ListenerOptions.once` at creation time — it drives the once-guard in `dispatch()` and avoids threading a parameter through every dispatch call. Move `dispatch()`, `invoke()`, `mark_fired()`, `set_app_error_handler_resolver()` methods from Listener. Add `HandlerInvoker.create(task_bucket, handler, kwargs, options, ...)` classmethod that constructs async wrapper, injector, rate limiter, and copies `options.once`. None of the moved methods (`dispatch`, `invoke`, `mark_fired`) contain logging calls — use the module-level `LOGGER` if logging is needed in the future.
...
**Step 2b: Add backward-compat property accessors on Listener** for all fields that moved to sub-structs. These are temporary — they allow T02 and T03 to pass tests before T04/T05 migrate all consumers to sub-struct paths. T05 removes them.

Add read-only `@property` accessors that delegate to sub-structs:

*Identity fields:* `owner_id`, `app_key`, `instance_index`, `name`, `source_tier`, `handler_name`, `handler_short_name`, `source_location`, `registration_source` — each returns `self.identity.<field>`.

*Options fields:* `once`, `debounce`, `throttle`, `timeout`, `timeout_disabled`, `priority` — each returns `self.options.<field>`.

*Invoker fields:* `orig_handler`, `error_handler`, `kwargs` — each returns `self.invoker.<field>`. Also `_async_handler`, `_injector`, `_rate_limiter`, `_fired`, `_app_error_handler_resolver` for framework-internal access.

*DurationConfig fields:* `duration`, `immediate`, `entity_id`, `is_attribute_listener`, `hold_predicate` — each returns `self.duration_config.<field> if self.duration_config else <default>` (None for optional fields, False for booleans). Also `_duration_timer` returning `self.duration_config._timer if self.duration_config else None`.

*Method forwards:* `dispatch()`, `invoke()`, `mark_fired()`, `set_app_error_handler_resolver()` — each delegates to `self.invoker.<method>(...)`.
...
- The logger field stays on Listener (not on sub-structs). The moved methods (`dispatch`, `invoke`, `mark_fired`) don't contain logging calls. Use the module-level `LOGGER` if logging is needed in the future.
...
- [ ] FR#3: HandlerInvoker groups handler callable, async wrapper, injector, kwargs, error handler, app error handler resolver, rate limiter, once flag, and fired guard; owns dispatch(), invoke(), mark_fired(), set_app_error_handler_resolver()
