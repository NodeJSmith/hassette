...
- The `LogTable` component currently manages search state internally via `useSignal` in `log-table.tsx:47`. To expose search to `LogsPage` for the `TableCard` search slot, add `search` and `onSearchChange` props to `LogTable`. When provided, `LogTable` uses the external state; when omitted (embedded uses), it falls back to its internal `useSignal`. The `useLogFilters` hook already returns `setSearch` — wire the external `onSearchChange` to it.
