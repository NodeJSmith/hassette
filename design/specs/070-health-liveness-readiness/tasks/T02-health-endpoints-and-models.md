...
3. **`src/hassette/web/routes/health.py`** — reshape and extend:
   - `GET /api/health` — remove the `if status_data.status == "starting": response.status_code = 503` rule (`health.py:19-21`). The handler returns the `SystemStatusResponse` body and sets no status code → always 200 while serving. Drop the `503` entry from `responses=` for this route.
