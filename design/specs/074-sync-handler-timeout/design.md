...
**Mechanism (committed, not deferred):** `run_in_thread` records the worker thread object in a `ContextVar` (or a small mutable cell captured in the `_call` closure) at the moment `_call` begins executing on the worker. When the timeout fires, the command layer reads that thread reference and checks `thread.is_alive()` — `True` after the await is cancelled means the worker outlived the timeout. No `concurrent.futures.Future` handle is threaded up through `make_async_adapter` to `_execute`; the command layer sees only a thread-liveness boolean, keeping it decoupled from the executor. The check is best-effort by nature (the thread may finish microseconds later); a brief grace read is acceptable. For executions where `_call` never started (the worker had not dequeued the job when the timeout fired — see Edge Cases), the thread reference is unset and the leak marker is not raised — a not-started timeout is not a leak.
...
Mirror the existing rate-limited capacity-warning pattern already used for the write queue (`command_executor.py:328-340`) and timeouts (`log_timeout_rate_limited`, `command_executor.py:290-326`). Read active vs. `max_workers` from the dedicated executor and emit a rate-limited WARNING when active crosses a threshold (e.g. 75%, matching `_CAPACITY_WARN_THRESHOLD`).
