...
2. **Capture the worker thread identity** inside `_call`, at the moment it begins executing on the worker: record `threading.current_thread()` into a ContextVar (e.g. a module-level `CURRENT_SYNC_WORKER: ContextVar[Thread | None]`) or into a small mutable cell returned/exposed to the caller. The capture must be readable by `command_executor._execute` at the timeout site (T06 consumes it). Choose the lowest-coupling mechanism: a ContextVar set inside `_call` and read at the timeout site avoids threading a future handle up through `make_async_adapter`. The reference is unset until `_call` actually starts running (so a timeout that fires before the worker dequeues the job leaves it unset — that is a "not-started" timeout, not a leak; T06 relies on this).
...
- The ContextVar must be set on the *worker* thread inside `_call`, and read on the *loop* thread at the timeout site. ContextVars copy into `run_in_executor` callables; confirm the value set inside `_call` is observable where T06 needs it, or use an explicit shared cell keyed per-execution if ContextVar propagation doesn't reach the reader. Document the chosen mechanism so T06 can consume it.
