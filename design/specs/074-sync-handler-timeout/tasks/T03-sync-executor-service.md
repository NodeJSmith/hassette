...
2. **Register in `src/hassette/core/core.py`** `wire_services()` (`:149-225`): add `self._sync_executor_service = self.add_child(SyncExecutorService)` early (it has no dependencies — place it alongside or before `DatabaseService` near `:163`). Expose `sync_executor` as a property on `Hassette` returning `self._sync_executor_service.executor` so `TaskBucket` (T04) can reach it via `self.hassette.sync_executor`.
