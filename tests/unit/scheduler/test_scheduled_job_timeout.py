...
class TestMatchesTimeoutAndJitter:
    def test_matches_returns_false_when_timeout_differs(self) -> None:
        ...
        job1 = make_job(job=noop, trigger=Every(hours=1), timeout=5.0)
        job2 = make_job(job=noop, trigger=Every(hours=1), timeout=10.0)
    ...
    def test_matches_returns_false_when_jitter_differs(self) -> None:
        ...
        job1 = make_job(job=noop, trigger=Every(hours=1), jitter=1.0)
        job2 = make_job(job=noop, trigger=Every(hours=1), jitter=5.0)
        assert not job1.matches(job2)

    def test_matches_returns_false_when_timeout_disabled_differs(self) -> None:
        """timeout_disabled change is detected by matches()."""
        job1 = make_job(job=noop, trigger=Every(hours=1), timeout_disabled=False)
        job2 = make_job(job=noop, trigger=Every(hours=1), timeout_disabled=True)
        assert not job1.matches(job2)

    def test_matches_returns_true_when_all_fields_match(self) -> None:
        """All fields including timeout and jitter match → True."""
        job1 = make_job(job=noop, trigger=Every(hours=1), timeout=5.0, jitter=2.0, timeout_disabled=False)
        job2 = make_job(job=noop, trigger=Every(hours=1), timeout=5.0, jitter=2.0, timeout_disabled=False)
...
class TestScheduledJobErrorHandlerField:
    ...
    def test_matches_with_same_error_handler(self) -> None:
        ...
        job1 = make_job(job=noop, trigger=Every(hours=1), error_handler=error_handler_a)
        job2 = make_job(job=noop, trigger=Every(hours=1), error_handler=error_handler_a)
        assert job1.matches(job2)

    def test_matches_with_different_error_handler(self) -> None:
        """matches() returns False when error_handler differs (identity check)."""
        job1 = make_job(job=noop, trigger=Every(hours=1), error_handler=error_handler_a)
        job2 = make_job(job=noop, trigger=Every(hours=1), error_handler=error_handler_b)
        assert not job1.matches(job2)
    ...
    def test_error_handler_compare_false_no_heap_corruption(self) -> None:
        ...
        """
        job1 = make_job(job=noop, trigger=Every(hours=1), error_handler=error_handler_a, name="job1")
        job2 = make_job(job=noop, trigger=Every(hours=1), error_handler=error_handler_b, name="job2")
