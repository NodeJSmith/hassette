...
class TestMatchesTriggerID:
    def test_matches_same_trigger_id(self) -> None:
        ...
        job2 = make_scheduled_job(job=noop, trigger=Every(hours=1))
    ...
    def test_matches_different_group(self) -> None:
        ...
        job1 = make_scheduled_job(job=noop, trigger=Every(hours=1), group="morning")
        job2 = make_scheduled_job(job=noop, trigger=Every(hours=1), group="evening")
        assert not job1.matches(job2)

    def test_matches_same_group(self) -> None:
        """Same callable, same trigger, same group → match."""
        job1 = make_scheduled_job(job=noop, trigger=Every(hours=1), group="morning")
        job2 = make_scheduled_job(job=noop, trigger=Every(hours=1), group="morning")
        assert job1.matches(job2)
