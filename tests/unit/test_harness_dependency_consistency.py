"""Unit test: harness DEPENDENCIES entries stay in sync with real service depends_on.

Tests:
- Topological sort function (unit tests for the string-based implementation)
- Bidirectional validation: harness DEPENDENCIES ⊆ real depends_on and vice versa
- Structural: _starters.keys() == set(COMPONENT_CLASS_MAP.keys()) | harness_only_components
"""
...
def test_harness_dependencies_subset_of_real() -> None:
    """Validates harness DEPENDENCIES ⊆ real depends_on."""
    for component_name, harness_deps in DEPENDENCIES.items():
        real_class = COMPONENT_CLASS_MAP.get(component_name)
        ...
        for dep_name in harness_deps:
            dep_class = COMPONENT_CLASS_MAP.get(dep_name)
            assert dep_class is not None, (
                f"Harness DEPENDENCIES[{component_name!r}] references {dep_name!r}, "
                f"but {dep_name!r} has no entry in COMPONENT_CLASS_MAP. "
                f"Either add the mapping or update DEPENDENCIES."
            )
            ...
            assert match_found, (
                ...
                f"contains no matching type.  Update either DEPENDENCIES in harness.py or the "
                f"real service's depends_on ClassVar."
            )


def test_real_dependencies_subset_of_harness() -> None:
    """Validates real depends_on ⊆ harness DEPENDENCIES (for mapped components)."""
    for component_name, real_class in COMPONENT_CLASS_MAP.items():
        harness_deps = DEPENDENCIES.get(component_name, set())

        for dep_type in real_class.depends_on:
            dep_name = REVERSE_CLASS_MAP.get(dep_type)
            if dep_name is None:
                continue
...
def test_starters_match_component_map() -> None:
    ...
    expected = set(COMPONENT_CLASS_MAP.keys()) | harness_only_components
