...
class TestScheduleEntryPoint:
    def test_schedule_creates_job_with_trigger(self) -> None:
        ...
        job = scheduler.schedule(noop, trigger)
    ...
    def test_schedule_group_tracked(self) -> None:
        ...
        scheduler = make_scheduler()
        job = scheduler.schedule(noop, Daily(at="07:00"), group="morning")
...
class TestCancelGroup:
    def test_cancel_group_cancels_all_members(self) -> None:
        ...
        scheduler = make_scheduler()
        job1 = scheduler.schedule(noop, Every(hours=1), name="job1", group="morning")
        job2 = scheduler.schedule(noop, Every(hours=2), name="job2", group="morning")
...
class TestListJobs:
    def test_list_jobs_no_filter(self) -> None:
        ...
        scheduler = make_scheduler()
        job1 = scheduler.schedule(noop, Every(hours=1), name="a", group="g1")
        job2 = scheduler.schedule(noop, Every(hours=2), name="b", group="g1")
        job3 = scheduler.schedule(noop, Every(hours=3), name="c", group="g2")
    ...
    def test_list_jobs_group_filter(self) -> None:
        ...
        scheduler = make_scheduler()
        job1 = scheduler.schedule(noop, Every(hours=1), name="a", group="g1")
        scheduler.schedule(noop, Every(hours=2), name="b", group="g2")
...
class TestJobsByGroupMaintenance:
    ...
    def test_dequeue_job_leaves_other_group_members(self) -> None:
        ...
        scheduler = make_scheduler()
        job1 = scheduler.schedule(noop, Every(hours=1), name="a", group="g")
        job2 = scheduler.schedule(noop, Every(hours=2), name="b", group="g")
    ...
    def test_removal_callback_called_on_exhaustion(self) -> None:
        ...
        scheduler = make_scheduler()
        job = scheduler.schedule(noop, After(seconds=30), group="once_group")
...
class TestConvenienceWrappers:
    def test_run_daily_delegates_to_daily_trigger(self) -> None:
        ...
        scheduler = make_scheduler()
        job = scheduler.run_daily(noop, at="07:00")
    ...
    def test_run_hourly_delegates_to_every_trigger(self) -> None:
        ...
        scheduler = make_scheduler()
        job = scheduler.run_hourly(noop, hours=2)
        assert isinstance(job.trigger, Every)
    ...
    def test_run_once_string_form_accepted(self) -> None:
        ...
        scheduler = make_scheduler()
        job = scheduler.run_once(noop, at="23:59")
...
class TestScheduleTypeError:
    def test_schedule_raises_typeerror_for_non_protocol_trigger(self) -> None:
        ...
        with pytest.raises(TypeError, match="trigger must implement TriggerProtocol"):
            scheduler.schedule(noop, NotATrigger())
...
class TestCancelJob:
    ...
    def test_cancel_job_rejects_wrong_scheduler(self) -> None:
        ...
        job = scheduler_a.schedule(noop, Every(hours=1), name="job1")
...
class TestIdentityPassThrough:
    ...
    def test_source_tier_assertion_rejects_invalid_value(self) -> None:
        ...
        with pytest.raises(AssertionError, match="Invalid source_tier"):
            scheduler.schedule(noop, Every(hours=1))
