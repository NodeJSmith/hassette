...
_IDENTITY_FIELDS: set[str] = {f.name for f in dataclasses.fields(ListenerIdentity)}

# All field names on ListenerOptions
_OPTIONS_FIELDS: set[str] = {f.name for f in dataclasses.fields(ListenerOptions)}

# Fields covered by either sub-struct
_COVERED_FIELDS: set[str] = _IDENTITY_FIELDS | _OPTIONS_FIELDS
...
def test_listener_registration_fields_have_sub_struct_source() -> None:
    ...
    (b) add it to _EXEMPTIONS with a comment explaining why it cannot be on a sub-struct.
    ...
    non_exempted = reg_fields - _EXEMPTIONS

    missing = non_exempted - _COVERED_FIELDS
    assert not missing, (
        f"ListenerRegistration fields have no source on ListenerIdentity or ListenerOptions: "
        f"{sorted(missing)}. "
        f"Either add the field to the appropriate sub-struct, or add it to _EXEMPTIONS in this test "
...
def test_exemptions_are_not_vacuous() -> None:
    ...
    Prevents stale entries in _EXEMPTIONS from silently masking regressions.
    """
    reg_fields = {f.name for f in dataclasses.fields(ListenerRegistration)}
    stale = _EXEMPTIONS - reg_fields
    assert not stale, (
        f"_EXEMPTIONS contains fields not present on ListenerRegistration: {sorted(stale)}. "
        f"Remove them from _EXEMPTIONS."
    )
...
def test_identity_fields_are_non_empty() -> None:
    ...
    """
    assert len(_IDENTITY_FIELDS) > 0, (
        "_IDENTITY_FIELDS is empty. The parity test would pass vacuously. Investigate ListenerIdentity field discovery."
    )
...
def test_options_fields_are_non_empty() -> None:
    ...
    """
    assert len(_OPTIONS_FIELDS) > 0, (
        "_OPTIONS_FIELDS is empty. The parity test would pass vacuously. Investigate ListenerOptions field discovery."
    )
