"""Ordering guarantee tests for Bus synchronous registration.
...
async def test_cancel_then_add_routes_exactly_one_handler(bus: "Bus") -> None:
    ...
    that could interleave. With synchronous registration both operations complete
    before the next line of caller code runs.
    """
    topic = "test.ordering.cancel_replace"

    # Register original handler
    sub = await bus.on(topic=topic, handler=handler_alpha, name="original")
    ...
    # Cancel, then immediately register replacement
    sub.cancel()
    sub2 = await bus.on(topic=topic, handler=handler_beta, name="replacement")
...
async def test_cancel_then_add_different_topics_both_routed(bus: "Bus") -> None:
    ...
    sub_a = await bus.on(topic="test.ordering.topic_a", handler=handler_alpha, name="alpha")
    sub_b = await bus.on(topic="test.ordering.topic_b", handler=handler_beta, name="beta")  # noqa: F841

    # Cancel topic_a handler
    sub_a.cancel()

    # Immediately add a new handler on topic_a
    await bus.on(topic="test.ordering.topic_a", handler=handler_gamma, name="topic_a_replacement")
...
async def test_cancel_multiple_then_add_replacement(bus: "Bus") -> None:
    ...
    sub1 = await bus.on(topic=topic, handler=handler_alpha, name="h1")
    sub2 = await bus.on(topic=topic, handler=handler_beta, name="h2")

    # Both present
    assert len(bus.get_listeners()) == 2

    # Cancel both, then add one replacement
    sub1.cancel()
    sub2.cancel()
    sub3 = await bus.on(topic=topic, handler=handler_gamma, name="h3")
...
async def test_remove_all_listeners_then_query_is_empty(bus: "Bus") -> None:
    ...
    await bus.on(topic=topic, handler=handler_beta, name="r2")
    await bus.on(topic="test.ordering.bulk_remove_2", handler=handler_gamma, name="r3")
...
async def test_register_after_bulk_remove_works(bus: "Bus") -> None:
    ...
    sub = await bus.on(topic="test.ordering.post_remove", handler=handler_alpha, name="first")
    listeners = bus.get_listeners()
...
async def test_interleaved_add_remove_preserves_count(bus: "Bus") -> None:
    ...
    sub2 = await bus.on(topic="test.ordering.interleaved", handler=handler_beta, name="i2")
    assert len(bus.get_listeners()) == 2
