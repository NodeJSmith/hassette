...
from .conftest import make_log_record, set_ws_state
...
class TestHealthEndpoints:
    ...
    async def test_health_returns_200_when_degraded(self, client: "AsyncClient", mock_hassette) -> None:
        ...
        set_ws_state(mock_hassette, connected=False, ever_connected=True)
        response = await client.get("/api/health")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_health_returns_200_when_starting(self, client: "AsyncClient", mock_hassette) -> None:
        ...
        set_ws_state(mock_hassette, connected=False, ever_connected=False)
        response = await client.get("/api/health")
        assert response.status_code == 200
        data = response.json()
