...
class TestOnlyAppsConfigFilter:
    ...
    async def test_only_apps_outranks_the_decorator(self, monkeypatch: pytest.MonkeyPatch):
        """A `@only_app`-decorated app stays blocked when --app names a different app."""
        chosen = create_app_manifest(suffix="chosen", app_dir=self.app_dir)
        decorated = create_app_manifest(suffix="decorated", app_dir=self.app_dir)
        write_test_app_with_decorator(app_file=chosen.full_path, class_name=chosen.class_name)
        write_test_app_with_decorator(app_file=decorated.full_path, class_name=decorated.class_name, has_only_app=True)

        monkeypatch.setenv("HASSETTE__ONLY_APPS", json.dumps([chosen.app_key]))

        chosen_running = asyncio.Event()
        await wire_up_app_running_listener(self.hassette.bus, chosen_running, chosen.app_key)

        write_app_toml(self.toml_file, app_dir=self.app_dir, apps=[chosen, decorated])
        await emit_file_change_event(self.hassette, {self.toml_file, chosen.full_path, decorated.full_path})

        with anyio.fail_after(3):
            await chosen_running.wait()

        assert self.app_handler.registry.get(chosen.app_key, 0) is not None
        assert self.app_handler.registry.get(decorated.app_key, 0) is None
        assert self.app_handler.registry.only_apps == frozenset({chosen.app_key})
...
class TestOnlyAppDecorator:
    ...
    async def test_hot_reload_adds_only_app_decorator(self):
        ...
        assert self.app_handler.registry.only_apps == frozenset()
        ...
        assert self.app_handler.registry.only_apps == frozenset({app3.app_key})
    ...
    async def test_only_app_persists_across_config_changes(self):
        ...
        assert self.app_handler.registry.only_apps == frozenset({only.app_key})
