...
async def test_bus_emit_delivers_to_subscriber(hassette_with_bus: "HassetteHarness") -> None:
    ...
    await bus.emit("test.emit_topic", BroadcastPayload(value="hello", count=42))

    await asyncio.wait_for(done.wait(), timeout=HANDLER_TIMEOUT)

    assert len(received) == 1
    event = received[0]
    assert event.topic == "test.emit_topic"
    assert isinstance(event.payload, HassettePayload)
    assert event.payload.data == BroadcastPayload(value="hello", count=42)


async def test_bus_emit_event_data_di_accessor(hassette_with_bus: "HassetteHarness") -> None:
    """A handler annotated with D.EventData[T] receives the pre-extracted typed data."""
    bus = hassette_with_bus.bus
    received_data: list[BroadcastPayload] = []
    done = asyncio.Event()

    async def handler(data: D.EventData[BroadcastPayload]) -> None:
        received_data.append(data)
        hassette_with_bus.task_bucket.post_to_loop(done.set)

    await bus.on(topic="test.di_topic", handler=handler, name="test_di_subscriber")

    await bus.emit("test.di_topic", BroadcastPayload(value="typed", count=7))

    await asyncio.wait_for(done.wait(), timeout=HANDLER_TIMEOUT)

    assert len(received_data) == 1
    assert received_data[0] == BroadcastPayload(value="typed", count=7)


async def test_bus_emit_self_delivery(hassette_with_bus: "HassetteHarness") -> None:
    """An app that emits and subscribes to the same topic receives its own event."""
    bus = hassette_with_bus.bus
    ...
    await bus.emit("test.self_delivery", BroadcastPayload(value="self", count=1))

    await asyncio.wait_for(done.wait(), timeout=HANDLER_TIMEOUT)

    assert len(received) == 1
    assert received[0] == BroadcastPayload(value="self", count=1)


async def test_bus_sync_emit_delivers_event(hassette_with_bus: "HassetteHarness") -> None:
    """bus.sync.emit delivers the event from a synchronous context."""
    bus = hassette_with_bus.bus
    received: list[BroadcastPayload] = []
    done = asyncio.Event()
    ...
    await loop.run_in_executor(
        None,
        lambda: bus.sync.emit("test.sync_emit_topic", BroadcastPayload(value="sync", count=99)),
    )

    await asyncio.wait_for(done.wait(), timeout=HANDLER_TIMEOUT)

    assert len(received) == 1
    assert received[0] == BroadcastPayload(value="sync", count=99)
...
def test_app_has_no_send_event() -> None:
    """App.send_event and AppSync.send_event_sync were removed — use bus.emit instead."""
    assert not hasattr(App, "send_event"), "App.send_event was removed — use bus.emit"
    assert not hasattr(AppSync, "send_event_sync"), "AppSync.send_event_sync was removed — use bus.emit"
