"""Unit tests for the docstring builder and the strings.json key-reference resolver.
...
from hassette_codegen.extractors.services import _resolve_key_ref
...
class TestBuildMethodDocstring:
    def test_no_params_no_returns_is_summary_only(self) -> None:
        doc = build_method_docstring("Call the cover.open_cover service.", [], returns_none=False)
        assert doc.startswith('        """Call the cover.open_cover service.')
        assert "Args:" not in doc
        assert "Returns:" not in doc

    def test_returns_none_adds_returns_section(self) -> None:
        doc = build_method_docstring("Call the cover.open_cover service synchronously.", [], returns_none=True)
        assert "Returns:\n            None." in doc
        assert "Args:" not in doc
    ...
    def test_documented_params_render_args(self) -> None:
        ...
        doc = build_method_docstring("Call the cover.set_cover_position service.", params, returns_none=False)
        assert "        Args:" in doc
    ...
    def test_params_without_description_are_omitted_from_args(self) -> None:
        ...
        doc = build_method_docstring("Call the cover.set_cover_position service.", params, returns_none=True)
        assert "Args:" not in doc
    ...
    def test_long_description_wraps_within_line_length(self) -> None:
        ...
        doc = build_method_docstring("Call the light.turn_on service.", params, returns_none=False)
        ...
        continuation = [ln for ln in doc.splitlines() if ln.startswith(" " * 16)]
        assert continuation
