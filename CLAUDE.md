...
**Bus** (`src/hassette/bus/`) - Event pub/sub with filtering. Methods: `on_state_change`, `on_attribute_change`, `on_call_service`, `on`. `wait_for(topic, *, where=None, timeout, name=None) -> Event[Any]` is a one-shot await primitive for sequential automation logic — it suspends the caller until a matching event is dispatched and returns that event directly, with no handler and no `Subscription` to manage. It only matches events dispatched after the call is awaited (pre-existing state never resolves it), raises `asyncio.TimeoutError` on expiry and `CancelledError` on shutdown/cancellation, and composes with `call_service` via the documented arm-before-fire pattern (`docs/pages/core-concepts/bus/methods.md`). All registration methods are `async` and must be awaited. `name=` is required on every DB-registered listener — omitting it raises `ListenerNameRequiredError` at call time. Supports glob patterns, predicates, conditions, debounce, throttle. The internal `Listener` dataclass composes four sub-structs: `ListenerIdentity` (ownership/telemetry fields), `ListenerOptions` (behavioral timing parameters), `HandlerInvoker` (handler invocation, dispatch, rate limiting), and `DurationConfig` (duration-hold configuration and timer lifecycle). Registration is synchronous with the DB — `sub.listener.db_id` is a valid integer immediately when the awaited call returns. `Subscription` no longer has a `registration_task` field.

Handler signatures are flexible: `HandlerInvoker.create()` builds a `ParameterInjector` from the handler's actual signature (`get_typed_signature`) and only injects the parameters it declares — a handler for an empty-payload event (e.g. `HassetteSimpleEvent`/`HassetteEmptyPayload`) can omit the `event` parameter entirely and take no arguments, same as `RuntimeQueryService.on_ws_connected`/`on_ws_disconnected`/`on_app_manifests_changed`. Don't pad a handler's signature with an unused `event: Event[Any]` param just to match sibling handlers that do consume payload data.
...
Every successful `Resource`/`Service` shutdown attempt returns an immutable `TeardownReport` (`src/hassette/resources/teardown.py`), and `resource.teardown_report` exposes the current unconsumed one. When the coordinator itself raises outside the shutdown body (observing a pending initializer, requesting shutdown), it stores that report before re-raising instead of returning it — `await resource.shutdown()` raises in that case, so read `resource.teardown_report` after catching the exception to see the same evidence a normal completion would have returned. `is_restart_safe` is derived from the report's recorded `TeardownCause` values, never stored directly — `True` only when a completed attempt recorded zero causes. `ResourceStatus.STOPPED` is unaffected by any of this: it still means lifecycle orchestration reached its terminal phase, and a resource can be `STOPPED` with a report whose `is_restart_safe` is `False`. Only the report — never `STOPPED` — decides whether `restart()`, `start()`, or a direct `initialize()` may run again; a report with `is_restart_safe` `False` raises `RestartRefusedError` and has no in-process reset path, including from test-reset helpers.

`LifecycleMixin` owns exactly one paired attempt per resource: `_init_task` is authoritative for every initialization path (`start()`'s joiner, a direct `initialize()` call, and `restart()`), and `_shutdown_task` is authoritative for every shutdown call. Concurrent or repeated callers join the existing task through `asyncio.shield()` instead of starting a second attempt; a repeated `shutdown()` after completion returns the stored report without rerunning hooks. Both tasks are created directly via `asyncio.Task(...)` (bypassing the loop's TaskBucket-attributing task factory — see `_create_lifecycle_task()` in `lifecycle.py`), not through `task_bucket.spawn()`: the shutdown body cancels TaskBucket work, so owning the coordinator inside that same bucket would make it cancel itself. `_shutdown_body_task` is a third, non-admission field — it never gates whether a lifecycle operation may start, but keeps a cancellation-resistant shutdown body reachable and exception-observed until it actually completes, even after the coordinator has already returned a report to its callers. Every public lifecycle front door (`initialize()`, `start()`, `restart()`, `shutdown()`) rejects a call made from its own active `_init_task`, `_shutdown_task`, or `_shutdown_body_task` with `LifecycleReentryError` before joining, cancelling, or creating anything — a hook cannot recursively drive its own owner's lifecycle.

### Process Model

Hassette is designed for exactly one live `Hassette()` instance per process at a time. Constructing a second instance *after* an earlier one has finished (or without ever running one concurrently with another) is fine and routine — the test suite does this constantly across test functions (`HassetteHarness`/`_TestableHassette`). What's unsupported is two `Hassette()` instances initializing or running concurrently in the same process. A few places carry process-global state for exactly this reason — `enable_basic_logging()`'s extra-logger snapshot/restore tracking (`src/hassette/logging_.py`) and `block_io_guard.py`'s patch-tracking globals both assume single-writer, not concurrent, access. Don't add concurrency guards (locks, owner-id checks, etc.) to state like this without a specific driving need — the assumption is a deliberate, documented non-goal, not an oversight.
...
### `file-sizes` / `duplicate-code` checks — a PR-level FAILURE is always real

The `file-sizes` and `duplicate-code` jobs in `.github/workflows/lint.yml` each run two steps with different scopes — don't conflate them when triaging CI status:

- **The new-code gate step** (`Check for new file-size regressions` / `Check duplicate code (new-code gate)`, PR events only) has no `continue-on-error` and is the step that actually determines the job's conclusion on a PR. It fails only when the PR's own diff made an already-oversized file bigger, created a new oversized file, or introduced a new 3+-way duplicate block (majority-overlap with the PR's added lines; see `tools/check_duplicate_code.py`'s `new_code_violations()`). **A FAILURE here — i.e. `file-sizes` or `duplicate-code` showing red anywhere in a PR's checks — is a real, in-scope regression introduced by that PR.** Fix it in-PR per `design-completeness.md`/`clean-code-findings.md` norms, the same as any other required check. Never treat it as baseline noise to skip.
- **The backlog step** (`Check file sizes (full backlog)` / `Check duplicate code (full backlog)`) tracks the real, already-triaged historical backlog (see the `Decompose *` issues and #1573 on the issue tracker) rather than anything a given PR broke. It genuinely still fails on pushes to `main` today (confirmed via CI job logs) — but that failure **never surfaces as a PR check**: it runs with `continue-on-error: true` for `file-sizes`, and for `duplicate-code` it doesn't run on PR events at all (`if: github.event_name != 'pull_request'`, since the gate step below already does its own head-tree scan and reports the backlog for context). Either way, its result cannot affect the job conclusion a PR shows.

Net effect: on a PR, both checks pass as long as that PR doesn't make things worse, and only then — there is no PR-visible "expected failure" from historical backlog to filter out. When triaging PR CI status (e.g. via `/mine-address-pr-issues`), a red `file-sizes` or `duplicate-code` check always means the current PR regressed something and blocks merge; treat it like any other required check, never as pre-existing debt. See `tools/check_file_size_regressions.py` and `tools/check_duplicate_code.py --gate-new-code`'s module docstrings for the full design.
