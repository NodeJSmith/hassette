...
Every `Resource`/`Service` shutdown attempt returns an immutable `TeardownReport` (`src/hassette/resources/teardown.py`), and `resource.teardown_report` exposes the current unconsumed one. `restart_safety` is derived from the report's recorded `TeardownCause` values, never stored directly — `RestartSafety.SAFE` only when a completed attempt recorded zero causes. `ResourceStatus.STOPPED` is unaffected by any of this: it still means lifecycle orchestration reached its terminal phase, and a resource can be `STOPPED` with an `UNSAFE` report. Only the report — never `STOPPED` — decides whether `restart()`, `start()`, or a direct `initialize()` may run again; an `UNSAFE` report raises `RestartRefusedError` and has no in-process reset path, including from test-reset helpers.
