...
`BusService` and `SchedulerService` both declare `depends_on: [DatabaseService, SyncExecutorService]` — the database is guaranteed ready before any listener or job registration can occur, and the dedicated sync-handler executor (`SyncExecutorService`) outlives every component that submits sync work, so it is torn down only after them at shutdown.
