...
`WebsocketService` marks itself lifecycle-ready unconditionally in `on_initialize()`, before `serve()` ever attempts a connection — lifecycle readiness ("the service is running") is intentionally decoupled from HA connection status ("the WebSocket handshake succeeded"). Connection status is tracked separately via `is_connected` (current) and `has_ever_connected` (latch, never reverts). Because of this, `ApiResource`, `StateProxy`, and `AppHandler` no longer list `WebsocketService` in their `depends_on` — `ApiResource.depends_on` is empty, `StateProxy.depends_on` is `[ApiResource, BusService, SchedulerService]`, and `AppHandler.depends_on` is `[ApiResource, BusService, SchedulerService, StateProxy, SyncExecutorService]`. This lets the web dashboard (`WebApiService`) and app bootstrap proceed on schedule even when Home Assistant is unreachable: `StateProxy.on_initialize()` catches a failed initial `load_cache()` and marks ready with an empty state cache rather than blocking the wave. `RuntimeQueryService.get_system_status()` reads `ws.is_connected` (not `is_ready()`, which is now always true) together with `has_ever_connected` to report "starting" (never connected), "degraded" (connected before, not now), or "ok" (connected).
