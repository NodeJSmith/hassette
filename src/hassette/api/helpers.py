...
class HelperClient(Resource):
    ...
    # builtins.list: the method name "list" shadows the builtin in the class body,
    # so bare list[...] in annotations resolves to the method on Python < 3.14.

    @overload
    async def list(self, domain: Literal["input_boolean"]) -> builtins.list[InputBooleanRecord]: ...
    @overload
    async def list(self, domain: Literal["input_number"]) -> builtins.list[InputNumberRecord]: ...
    @overload
    async def list(self, domain: Literal["input_text"]) -> builtins.list[InputTextRecord]: ...
    @overload
    async def list(self, domain: Literal["input_select"]) -> builtins.list[InputSelectRecord]: ...
    @overload
    async def list(self, domain: Literal["input_datetime"]) -> builtins.list[InputDatetimeRecord]: ...
    @overload
    async def list(self, domain: Literal["input_button"]) -> builtins.list[InputButtonRecord]: ...
    @overload
    async def list(self, domain: Literal["counter"]) -> builtins.list[CounterRecord]: ...
    @overload
    async def list(self, domain: Literal["timer"]) -> builtins.list[TimerRecord]: ...

    async def list(self, domain: HelperDomain) -> builtins.list[Any]:
