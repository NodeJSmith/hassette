...
async def get_app_health_aggregates(
    ...
    Args:
        tqs: The ``TelemetryQueryService`` instance.
...
async def get_all_app_summaries(
    ...
    async with asyncio.timeout(tqs.hassette.config.database.read_timeout_seconds), tqs._snapshot_lock:
        try:
            await tqs._db.execute("BEGIN DEFERRED")
            async with tqs._db.execute(listener_reg_query) as cursor:
                listener_reg_rows = await cursor.fetchall()
            async with tqs._db.execute(listener_act_query, listener_act_params) as cursor:
                listener_act_rows = await cursor.fetchall()
            async with tqs._db.execute(job_reg_query) as cursor:
                job_reg_rows = await cursor.fetchall()
            async with tqs._db.execute(job_act_query, job_act_params) as cursor:
                job_act_rows = await cursor.fetchall()
        finally:
            with contextlib.suppress(aiosqlite.OperationalError):
                await tqs._db.execute("ROLLBACK")

    return _build_app_summaries(listener_reg_rows, listener_act_rows, job_reg_rows, job_act_rows, source_tier)


async def get_session_list(tqs: "Any", limit: int = 20) -> list[SessionRecord]:
    """Return recent session records.

    ``session_id`` is not exposed in the returned model (FR#18/AC#13);
    ``SessionRecord.id`` carries the integer row id but is not forwarded to
    any API-facing response model.
    """
    query = """
        SELECT
            s.id,
        ...
        LIMIT :limit
    """
    async with tqs.execute(query, {"limit": limit}) as cursor:
...
async def get_log_records(
    ...
    *,
    ...
    ``session_id`` is not included in the SELECT (FR#18/AC#13). All other
    log_records columns are returned as-is.
    """
    clauses: list[str] = []
    params: dict[str, Any] = {}
...
async def get_log_records_by_execution(
    ...
    *,
    ...
    async with tqs.execute(query, {"execution_id": execution_id, "limit": limit + 1}) as cursor:
