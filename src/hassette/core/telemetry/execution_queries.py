...
async def get_executions(
    ...
    *,
    ...
    Args:
        tqs: The ``TelemetryQueryService`` instance providing ``execute()``.
...
async def get_app_recent_activity(
    ...
    # row_id now carries the execution_id UUID (Step 7 of task spec)
    query = f"""
...
async def check_execution_predates_retention_cutoff(tqs: "Any", execution_id: str, cutoff: float) -> bool:
    """Check if an execution predates the retention cutoff.

    Queries the unified ``executions`` table - one query replaces the previous
    two-query pattern against ``handler_invocations`` and ``job_executions``.
    """
    async with tqs.execute(
