"""Token resolution for the web API's bearer-token/session-cookie auth fallback.

Resolves the credential used by ``AuthDep``/the default-deny middleware (built in later
tasks) to validate ``Authorization: Bearer <token>`` headers and mint session cookies. See
