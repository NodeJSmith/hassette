...
class HassetteHarness:
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            # contextvars.Context than the one active when the Token was created — e.g. a
            # synchronous request.addfinalizer() callback resuming a module-scoped harness
            # fixture (see tests/integration/conftest.py::hassette_instance for the general
            # mechanism this mirrors). ContextVar.reset() across Context boundaries is invalid
            # by design; the mutation it would undo was scoped to that now-finished setup
            # Task's own context copy anyway, so there is nothing left to leak. Suppress and
            # continue so the task-factory restore below still runs even when this happens.
