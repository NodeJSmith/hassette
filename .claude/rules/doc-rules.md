...
AI-generated docs have specific tells. Watch for these:

- **Copula avoidance.** "Serves as", "acts as", "functions as" when you mean "is." The bus *is* a pub/sub system, it doesn't "serve as" one.
- **Significance inflation.** "Pivotal", "crucial", "fundamental", "robust." If everything is crucial, nothing is. Use plain words.
- **Dangling -ing phrases.** "Highlighting the importance of...", "Ensuring that handlers..." These add words without adding meaning. Delete them or expand into a real sentence.
- **Synonym cycling.** Don't call it a "handler", then a "callback", then a "listener function", then a "subscriber" in the same section. Pick one term and repeat it.
- **Filler hedging.** "It is important to note that" means nothing. "In order to" is just "to." "Due to the fact that" is just "because."
- **Abstract metaphor nouns.** "Leverage", "utilize", "facilitate", "surface" (as a verb). Use "use", "use", "help", "show."
...
- **Scope both runs to the pages you changed**, not the whole section. Pass the page slugs (e.g. `core-concepts/api/methods`) so the reviews target your edits, not unrelated pages.
...
- **Persona: a `lost` or `stuck-at-step-N` verdict on new content is a blocker.** Fix it before shipping. `followable` and `followable-with-effort` are passing verdicts.
- **Accuracy: a confirmed `WRONG` or `OUTDATED_API` finding on touched lines is a blocker.** Triage each finding against the cited code first — the reviewer can be wrong, and a bad "fix" makes a true sentence false — then fix the ones that hold before shipping.

This applies to docs-site prose pages. It does not apply to docstring-only changes (the reviews read rendered pages, not API reference generated from docstrings) or to `design/`, `.claude/`, and other non-site documentation.
